...
<Plotly
	...
	height={400}
	caption="Per-type definition cost: cold (recompile to bytecode), warm (cached bytecode), and mypyc-compiled."
/>
...
<Plotly
	{...charts.instCost}
	height={400}
	caption="Per-instance construction time, interpreted vs mypyc-compiled."
/>
...
<Plotly
	{...charts.immMemory}
	height={380}
	caption="Memory: freezing never changes the footprint — frozen=True is a write-path flag, not a layout change."
/>
...
<Plotly
	...
	height={380}
	caption="Construction: freezing adds a write-path cost for dataclass and attrs (each field through object.__setattr__), and is essentially free for msgspec."
/>
...
The `Final` is for the static checker, meaning that it has zero runtime cost.[^9] `mypy` rejects `o.a = 99`, but the assignment succeeds anyway, on the
...
[^9]: ["typing.Final"](https://docs.python.org/3/library/typing.html#typing.Final). docs.python.org. Retrieved 2026-06-21. "There is no runtime checking of these properties." (See also [PEP 591](https://peps.python.org/pep-0591/).)
...
**~19 ms**, because it's a serialization library and you can't get just the struct without importing the whole kitchen sink.[^8]
...
- **mypyc axis.** The generated module is compiled with `mypyc` and the resulting
  ...
  cost is barely lower than interpreted, likely because type creation is dominated by
